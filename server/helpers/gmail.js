...
//   const mailOptions = {
//     from: `"Butterfly Nepal" <${process.env.EMAIL_USER}>`,
...
//         <hr>
//         <p style="font-size: 12px; color: #777;">This order was placed from your Butterfly Nepal website.</p>
//       </div>
//     `,
//   };
 ...
 * @param {Object} order - Order object containing customer & cart info
 */
const sendOrderConfirmationEmail = async (order) => {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <p style="margin-top: 20px;"><strong>Total Amount:</strong> Rs. ${
        order?.totalAmount
      }</p>
      <hr>
      <p style="font-size: 12px; color: #777;">This order was placed from your Butterfly Nepal website.</p>
  ...
  const msg = {
    to: recipients,
    from: {
      name: "Butterfly Nepal",
      email: process.env.FROM_EMAIL,
    },
    subject: `🛍️ New COD Order - ${
      order?.addressInfo?.fullName || "Customer"
    }`,
    html: htmlContent,
  };
  ...
  try {
    ...
    console.log("✅ Order email sent successfully to:", recipients);
  } catch (error) {

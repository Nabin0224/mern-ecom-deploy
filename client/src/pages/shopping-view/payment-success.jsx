    ...
    <>
      <Card className="h-auto w-4/5 md:w-1/3 mx-auto border-2 border-gray-300 mt-4 md:mt-8 p-6   md:rounded-lg shadow-lg bg-white relative mb-2">
        <CardContent className="flex flex-col items-center gap-3">
          <div className="text-2xl md:text-3xl font-semibold text-green-600 mb-4">
            Payment Successful!
          </div>
          ...
          <div className="text-sm text-gray-500 mb-4 font-extralight ">
            ...
            {formData?.formData?.cartItem &&
            formData?.formData?.cartItem.length > 0
              ? formData.formData.cartItem.map((item) => (
                  <>
                    - Items: {item.quantity}x {item.title} <br />- Total:{" "}
                    {item.price} <br />
                  </>
                ))
              : null}
            ...
            <Separator className="h-[1px] bg-gray-400 my-6" />

            <span className="font-extralight text-xs italic opacity-70">
              ** Total amount is exclusive of delivery Charge. Delivery charges
